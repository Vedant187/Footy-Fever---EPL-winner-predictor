...
def compare_players(name1_ext,name2_ext):
    ...
    radar_output = radar.draw_radar_compare(values1, values2, ax=ax,
                                            kwargs_radar={'facecolor': '#000000', 'alpha': 0.6},
                                            kwargs_compare={'facecolor': '#FFFFFF', 'alpha': 0.6})
...
def compare_teams(team1_ext,team2_ext):
...
def analyzeteams(request):
    if(request.method == 'POST'):
        ...
        analysis_img = analyze_team(team_ext)
        buffered = BytesIO()
        analysis_img.save(buffered,format="JPEG")
        img_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
        context = {'image_str':img_str}
...
def analyzeplayers(request):
    if(request.method == 'POST'):
        ...
        analysis_img = analyze_player(name_ext)
        buffered = BytesIO()
        analysis_img.save(buffered,format="JPEG")
        img_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
        context = {'image_str':img_str}
...
def compareplayers(request):
    if(request.method == 'POST'):
        ...
        compare_players_image = compare_players(player1_ext,player2_ext)
        buffered = BytesIO()
...
def compareteams(request):
    if(request.method == 'POST'):
        ...
        compare_teams_image = compare_teams(team1_ext,team2_ext)
        buffered = BytesIO()
